# Getting Started with bot_army_memory

This guide walks you through setting up Memory Bot for local development.
...
DATABASE_NAME=bot_army_memory_dev
```
...
Memory Bot uses the **BotArmy.GenBot** harness to run skills. Skills are discrete, reusable units of work triggered via NATS messages.

### Creating Your First Skill

1. **Start with the template** — `lib/bot_army_memory/skills/example.ex` is a fully documented template
2. **Copy and rename** it for your skill:
   ```bash
   cp lib/bot_army_memory/skills/example.ex lib/bot_army_memory/skills/my_skill.ex
   ```
...
```elixir
defmodule BotArmyMemory.Skills.Classify do
  use BotArmy.Skill

  def name, do: :classify
  def description, do: "Classifies text into categories"
  def nats_triggers, do: ["bot.bot_army_memory.command.classify"]
...
```elixir
# In lib/bot_army_memory/application.ex or similar

defmodule BotArmyMemory.MyBot do
  use BotArmy.GenBot,
    skills: [
      BotArmyMemory.Skills.Classify,
      BotArmyMemory.Skills.ExtractEntities
    ],
    bot_id: :my_bot
end
```
...
```bash
# Make your changes
edit lib/bot_army_memory/skills/my_skill.ex
...
This creates a release named `memory_bot` that is deployed to `/opt/ergon/releases/memory_bot/` on the server.
...
3. Extracts and deploys to `/opt/ergon/releases/memory_bot/`
