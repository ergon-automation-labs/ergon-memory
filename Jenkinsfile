pipeline {
  ...
  environment {
    ...
    GITHUB_REPO = "ergon-automation-labs/ergon-memory"
  }
    ...
    stage('Run Migrations') {
      steps {
        sh '''
          ...
          RELEASE_BIN="${RELEASE_DIR}/current/memory_bot/bin/memory_bot"
          ...
          echo "Running: $RELEASE_BIN eval 'BotArmyMemory.Release.create()'"
          $RELEASE_BIN eval 'BotArmyMemory.Release.create()'

          echo "Running: $RELEASE_BIN eval 'BotArmyMemory.Release.migrate()'"
          $RELEASE_BIN eval 'BotArmyMemory.Release.migrate()'
  ...
  post {
    success {
      sh '''
        ...
        if [ -f ./release-artifact/bot_army_memory/releases/start_erl.data ]; then
          VERSION=$(awk '{print $2}' ./release-artifact/bot_army_memory/releases/start_erl.data)
        fi
